...
### ปัญหาเดิม
ก่อนจะมี Template Engine หากคุณต้องการส่งหน้าเว็บที่มีข้อมูลจาก Database ไปให้ User คุณต้องเขียนโค้ดแบบ "กรรมกร" มาก

- **ปัญหา String ต่อกันเป็นพรืด:** คุณต้องเอาตัวแปร JavaScript ไปบวกกับ String ของ HTML เช่น `ปัญหา String ต่อกันเป็นพรืด: คุณต้องเอาตัวแปร JavaScript ไปบวกกับ String ของ HTML เช่น:`
- **ความยุ่งยาก:** ถ้าหน้าเว็บมีโค้ด HTML 500 บรรทัด แล้วต้องเอาข้อมูลไปแทรกในนั้น โค้ดจะอ่านยากมาก และเสี่ยงต่อการพิมพ์ผิดสูง
- **จัดการ Logic ลำบาก:** การจะทำ Loop เพื่อแสดงรายการสินค้า 10 อย่างใน HTML ด้วย res.send แทบจะเป็นฝันร้ายของโปรแกรมเมอร์เลย
### หลักการทำงาน
Template Engine ทำหน้าที่เป็น "เครื่องปั่นข้อมูล":
1. Template: คุณเขียนไฟล์ HTML ที่มี "สัญลักษณ์พิเศษ" (Placeholders) เตรียมไว้
2. Data: คุณส่ง Object ข้อมูลจาก Express ไปให้
3. Render: Template Engine จะเอาข้อมูลไปเสียบแทนที่สัญลักษณ์เหล่านั้น แล้วพ่นออกมาเป็น HTML บริสุทธิ์ ส่งไปที่ Browser

### เนื้อหา เยอะไหม เรียนอะไรบ้าง
เนื้อหาไม่ได้เยอะ แต่มีตัวเลือกให้ใช้หลายตัว (เหมือนเลือกยี่ห้อรถ)

**ยอดนิยม**

**EJS (Embedded JavaScript)* นิยมที่สุด เพราะเขียนเหมือน HTML ปกติเลย แค่ใส่ `<%= %>` ครอบตัวแปร

**Pug (ชื่อเดิมคือ Jade):* เน้นเขียนสั้น ตัดเครื่องหมาย `< >` ออกหมด ใช้การย่อหน้าแทน (สะอาดตาแต่ต้องหัดใหม่)

**Handlebars (HBS):* เน้นความเรียบง่าย ใช้สัญลักษณ์ `{{ }}`



**หัวข้อที่ต้องฝึก**

Interpolation: วิธีการแสดงค่าตัวแปร

Conditionals: การใช้ if-else ในไฟล์ HTML (เช่น ถ้า Login แล้วให้โชว์รูปโปรไฟล์)

Loops: การวนลูปแสดงข้อมูล (เช่น แสดงตารางรายชื่อ user)

Partials/Layouts: การแยก Header และ Footer ไว้ที่ไฟล์เดียวแล้วดึงไปใช้ทุกหน้า (ไม่ต้องเขียนซ้ำ)


**สรุปการใช้งาน**
clean Code: แยกไฟล์ HTML (View) ออกจากไฟล์ Logic (Controller) ชัดเจนตามหลัก MVC

Maintainability: แก้ไขหน้าตาเว็บได้ง่าย ไม่กระทบโค้ดส่วนการทำงาน

Dynamic Content: สร้างหน้าเว็บที่ปรับเปลี่ยนตามข้อมูลผู้ใช้ได้รวดเร็วโดยไม่ต้องใช้ Framework ฝั่ง Client หนักๆ
...
โค้ดใน Express:
``` js
app.set('view engine', 'ejs'); // บอก Express ว่าเราจะใช้ EJS

app.get('/profile', (req, res) => {
    const userData = { name: "Somchai", age: 25 };
    res.render('profile', userData); // ส่งข้อมูลไปที่ไฟล์ profile.ejs
});
``` 
ในไฟล์ `profile.ejs`
``` js
<h1>ชื่อผู้ใช้: <%= name %></h1>
<p>อายุ: <%= age %></p>
```